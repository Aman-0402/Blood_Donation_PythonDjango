...
function HospitalProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      const result = profile ? await updateHospitalProfile(form) : await createHospitalProfile(form)
      setProfile(result)
    ...
    <form onSubmit={handleSubmit} className="max-w-xl space-y-4">
      <div className="flex items-center gap-3">
        <h1 className="text-2xl font-semibold text-gray-900">
          {profile ? 'Hospital profile' : 'Create hospital profile'}
        </h1>
      ...
      {profile && !profile.is_verified && (
        <Alert type="info">An administrator will review your license number and verify your hospital.</Alert>
      )}
      <FormField label="Hospital name" id="name" value={form.name} onChange={handleChange} required />
